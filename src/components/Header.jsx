...
const Header = ({ showChatbot }) => {
  ...
  // 쪽지 읽음 처리 시 알림 카운트 감소 (Message 컴포넌트에서 호출)
  ...
  useEffect(() => {
    window.decreaseUnreadCount = decreaseUnreadCount;
    return () => {
      delete window.decreaseUnreadCount;
    };
  }, []);
          ...
          case 'NOTIFICATION_READ':
            ...
            setNotifications((prev) =>
              prev.map((notification) => {
                const currentId =
                  notification.notificationId || notification.messageId;
                return currentId === data.notificationId
                  ? { ...notification, isRead: true }
                  : notification;
              }),
            );
